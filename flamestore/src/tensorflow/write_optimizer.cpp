#include "tensorflow/core/framework/op.h"
#include "tensorflow/core/framework/shape_inference.h"
#include "tensorflow/core/framework/op_kernel.h"
#include <iostream>
#include <functional>
#include "../client.hpp"

using namespace tensorflow;
using shape_inference::DimensionHandle;
using shape_inference::InferenceContext;
using shape_inference::ShapeHandle;

REGISTER_OP("WriteOptimizer")
    .Attr("client: string")
    .Attr("provider_addr: string")
    .Attr("provider_id: int")
    .Attr("model_name: string")
    .Attr("optimizer_signature: string")
    .Input("tensors: T")
    .Attr("T: list(type)")
    .SetIsStateful()
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
            return Status::OK();
    });


class WriteOptimizerOp : public OpKernel {

private:
    
    std::string          m_model_name;
    std::string          m_optimizer_signature;
    std::string          m_provider_addr;
    int                  m_provider_id;
    flamestore_client*   m_client;

    void validate_input(OpKernelContext* context) {}

public:

    explicit WriteOptimizerOp(OpKernelConstruction* context)
    : OpKernel(context) {
        OP_REQUIRES_OK(context, context->GetAttr("model_name",          &m_model_name));
        std::string client_id;
        OP_REQUIRES_OK(context, context->GetAttr("client",              &client_id));
        m_client = flamestore_client::from_id(client_id);
        OP_REQUIRES_OK(context, context->GetAttr("provider_addr",       &m_provider_addr));
        OP_REQUIRES_OK(context, context->GetAttr("provider_id",         &m_provider_id));
        OP_REQUIRES_OK(context, context->GetAttr("optimizer_signature", &m_optimizer_signature));
    }

    void Compute(OpKernelContext* context) override {

        validate_input(context);
        unsigned n = context->num_inputs();
        std::vector<std::pair<void*, size_t>> segments(n);
        for(unsigned i=0; i < n; i++) {
            auto& tensor = context->input(i);
            segments[i].first  = static_cast<void*>(
                    const_cast<char*>(tensor.tensor_data().data()));
            segments[i].second = tensor.tensor_data().size();
        }
       
        auto ph = m_client->lookup(m_provider_addr, m_provider_id);

        auto local_bulk = m_client->engine().expose(segments, tl::bulk_mode::read_only);

        auto status = m_client->write_optimizer_data(
                                    ph,
                                    m_model_name,
                                    m_optimizer_signature,
                                    local_bulk);
        OP_REQUIRES(context, status.first == 0,
                errors::Internal(status.second));
    }

};

REGISTER_KERNEL_BUILDER(Name("WriteOptimizer").Device(DEVICE_CPU), WriteOptimizerOp);
