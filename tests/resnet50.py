...
def load_dataset(train_file=None, test_file=None, 
    ...
    if(train_file is not None):
        if(train_set_x is None or train_set_y is None):
            print('Error in load_dataset: train_set_x and train_set_y must be specified')
            sys.exit(-1)
    ...
    if(test_file is not None):
        if(test_set_x is None or test_set_y is None):
            print('Error in load_dataset: test_set_x and test_set_y must be specified')
            sys.exit(-1)
...
def create_model(img_shape=(64,64), num_classes=6, dropout=0.7):
    ...
    If trainable is set to a number X, only the last X layers will be trainable.
    """
...
def train_model(model, dataset):
    callbacks = [RemoteCheckpointCallback('MyModel', 'connection.json')]
